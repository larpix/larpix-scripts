...
def plot_leakage_hist(results, figure_title='Leakage rate distribution', label='_nolegend_'):
    ...
    plt.hist(hist_leakage, alpha=1.0, label=label, bins='sqrt')
    leg = ax.legend(ncol=max(int(np.sqrt(n_chips)),1))
    ax.grid(True, linestyle='--', linewidth=1)
    ax.axis(option='auto')
    ax.set_frame_on(True)
...
def plot_pedestal_width_hist2d(results, figure_title='ADC distribution'):
    ...
    x_bins = np.linspace(-0.5, n_chips * AXIS_FORMAT_CHIP_SIZE + 0.5, n_chips * AXIS_FORMAT_CHIP_SIZE+2)
    y_bins = np.linspace(-0.5, MAX_ADC_VALUE + 0.5, (MAX_ADC_VALUE+2)//2)
    plt.hist2d(hist_channel_ids, hist_adc_values, bins=[x_bins, y_bins], alpha=1.0, label='_nolegend_', norm=LogNorm())
    cb = plt.colorbar()
    #leg = ax.legend(ncol=max(int(np.sqrt(n_chips)),1), loc=8)
    ax.grid(True, linestyle='--', linewidth=1)
    ax.axis(option='auto')
...
def plot_pedestal_width_mean_hist(results, figure_title='Pedestal mean', label='_nolegend_'):
    ...
    plt.hist(hist_adc_mean, alpha=1.0, label=label, bins='sqrt')
    leg = ax.legend()
    ax.grid(True, linestyle='--', linewidth=1)
    ax.axis(option='auto')
    ax.set_frame_on(True)
...
def plot_pedestal_width_rms_hist(results, figure_title='Pedestal width', label='_nolegend_'):
    ...
    plt.hist(hist_adc_rms, alpha=1.0, label=label, bins='sqrt')
    leg = ax.legend()
    ax.grid(True, linestyle='--', linewidth=1)
    ax.axis(option='auto')
    ax.set_frame_on(True)
...
def plot_trigger_threshold_hist2d(results, figure_title='Trigger threshold and efficiency'):
    ...
    hist = plt.hist2d(hist_channel_ids, hist_ampl_values, weights=hist_ampl_weights, bins=[x_bins, y_bins], alpha=1.0, label='_nolegend_', vmin=1e-6, vmax=MAX_EFFICIENCY)
    cb = plt.colorbar()
